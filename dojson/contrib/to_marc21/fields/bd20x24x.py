...
def reverse_uniform_title(self, key, value):
    ...
    if indicator_map1.get(value.get("uniform_title_printed_or_displayed"), "7") != "7":
        try:
    ...
    return {
        ...
        "$ind1": indicator_map1.get(
            value.get("uniform_title_printed_or_displayed"), "_"
        ),
        "$ind2": indicator_map2.get(value.get("nonfiling_characters"), "_"),
    }
...
def reverse_varying_form_of_title(self, key, value):
    ...
    if indicator_map1.get(value.get("note_added_entry_controller"), "7") != "7":
        try:
