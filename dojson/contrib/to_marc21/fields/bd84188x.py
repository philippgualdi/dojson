...
def reverse_location(self, key, value):
    ...
    return {
        ...
        "$ind1": "7"
        if "shelving_scheme" in value
        and not indicator_map1.get(value.get("shelving_scheme"))
        and value.get("shelving_scheme")
        == value.get("source_of_classification_or_shelving_scheme")
...
def reverse_electronic_location_and_access(self, key, value):
    ...
    return {
        ...
        "$ind1": "7"
        if "access_method" in value
        and not indicator_map1.get(value.get("access_method"))
        and value.get("access_method") == value.get("access_method")
...
def reverse_machine_generated_metadata_provenance(self, key, value):
    ...
    if indicator_map1.get(value.get("method_of_machine_assignment"), "7") != "7":
        try:
