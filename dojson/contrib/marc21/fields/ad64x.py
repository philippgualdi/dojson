...
@marc21_authority.over(
    "series_dates_of_publication_and_or_sequential_designation", "^640.."
)
...
def series_numbering_example(self, key, value):
    ...
    return {
        ...
        "field_link_and_sequence_number": utils.force_list(value.get("8")),
        "institution_copy_to_which_field_applies": utils.force_list(value.get("5")),
...
def series_place_and_publisher_issuing_body(self, key, value):
    ...
    return {
        "__order__": tuple(order) if len(order) else None,
        "place": utils.force_list(value.get("a")),
        "field_link_and_sequence_number": utils.force_list(value.get("8")),
        "publisher_issuing_body": utils.force_list(value.get("b")),
