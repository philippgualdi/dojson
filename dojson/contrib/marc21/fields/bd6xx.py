...
@marc21.over("subject_added_entry_uniform_title", "^630[9341750_268][341750_26]")
@utils.for_each_value
@utils.filter_values
...
def index_term_uncontrolled(self, key, value):
    ...
    return {
        "__order__": tuple(order) if len(order) else None,
        "uncontrolled_term": utils.force_list(value.get("a")),
        "field_link_and_sequence_number": utils.force_list(value.get("8")),
        "linkage": value.get("6"),
...
def subject_added_entry_faceted_topical_terms(self, key, value):
    ...
    return {
        "__order__": tuple(order) if len(order) else None,
        "chronological_subdivision": utils.force_list(value.get("y")),
        "facet_hierarchy_designation": utils.force_list(value.get("c")),
        "materials_specified": value.get("3"),
        "relator_code": utils.force_list(value.get("4")),
        "geographic_subdivision": utils.force_list(value.get("z")),
        "authority_record_control_number": utils.force_list(value.get("0")),
        "relator_term": utils.force_list(value.get("e")),
        "non_focus_term": utils.force_list(value.get("b")),
        "source_of_heading_or_term": value.get("2"),
        "linkage": value.get("6"),
        "focus_term": utils.force_list(value.get("a")),
        "field_link_and_sequence_number": utils.force_list(value.get("8")),
        "form_subdivision": utils.force_list(value.get("v")),
        "level_of_subject": indicator_map1.get(key[3]),
    }
...
def index_term_curriculum_objective(self, key, value):
    ...
    return {
        ...
        "subordinate_curriculum_objective": utils.force_list(value.get("b")),
