...
def test_xml_to_marc21_to_xml(file_name):
    ...
    with codecs.open(os.path.join(path, "data", file_name), "r", "utf-8") as myfile:
        expect = myfile.read()

    schema = pkg_resources.resource_filename(
        "dojson.contrib.marc21.schemas", "marc21/bibliographic/bd-v1.0.2.json"
    )
...
def test_xml_to_marc21_authority_to_xml(file_name):
    ...
    schema = pkg_resources.resource_filename(
        "dojson.contrib.marc21.schemas", "marc21/authority/ad-v1.0.2.json"
    )
...
def test_cli_do_marc21_from_xml():
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            cli.cli, ["-i", "record.xml", "-l", "marcxml", "missing", "marc21"]
        )
        ...
        result = runner.invoke(
            cli.cli, ["-i", "record.xml", "-l", "marcxml", "do", "--strict", "marc21"]
        )
        assert 0 == result.exit_code
...
def test_cli_do_marc21_from_xml_unknown_fields():
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            cli.cli, ["-i", "record_999.xml", "-l", "marcxml", "missing", "marc21"]
        )
...
def test_cli_do_marc21_from_json():
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(cli.cli, ["-i", "record.json", "missing", "marc21"])
        assert "" == result.output, result.exception
        assert 0 == result.exit_code

        result = runner.invoke(
            cli.cli, ["-i", "record.json", "do", "marc21", "schema", "/schema.json"]
        )
